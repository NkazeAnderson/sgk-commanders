...
export async function getUserByEmail(email:string) {
  const res =  await supabase.from(tables.users).select("*").eq("email", email).single();
  if (res.error) throw res.error;
  return usersSchema.parse(res.data);
}

export async function getUserByPhone(phone:number) {
  const res =  await supabase.from(tables.users).select("*").eq("phone", phone).single();
  if (res.error) throw res.error;
  return usersSchema.parse(res.data);
}

export async function getUserById(id:string) {
   const res = await supabase.from(tables.users).select("*").eq("id", id).single()
   if (res.error) throw res.error;
   return usersSchema.parse(res.data);
}
...
export async function updateUser( user:Partial<User> & {id:string}) {
    ...
    const { data, error } = await supabase.from(tables.users).update(rest).eq("id", id).select().single();
    if (error) throw error;
    return usersSchema.parse(data);
}
