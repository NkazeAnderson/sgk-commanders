...
import { DollarSign, Info, User } from 'lucide-react-native';
...
const ClientSubscriptionBanner = () => {
  ...
  const expired = user.subcriptionExpiration && new Date(user.subcriptionExpiration) < new Date();
  const noSubscription = !!user.subcriptionExpiration
  useEffect(()=>{
  ...
  return (
    <HStack>
        ...
        <Box className="rounded-sm bg-typography-50 px-2 ml-8 self-start">
          <Text size="xs" bold className=" text-primary-950">Subscription</Text>
        </Box>
        <HStack space="md" className="items-center ">
          <Icon as={User} className="text-primary-100"/>
          <Heading size='sm' className=' text-primary-100'>{ `${subscription?.name ?? "None"}`}</Heading>
        </HStack>
          {noSubscription ? (
...
export default ClientSubscriptionBanner